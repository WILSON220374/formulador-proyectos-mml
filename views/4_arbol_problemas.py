...
# --- ESTILO DE LA INTERFAZ (Vuelve a los tonos suaves con bordes limpios) ---
st.markdown("""
    <style>
    ...
    /* Espaciador invisible para nivelación de padres */
    .spacer-nivel {
        height: 190px;
        margin-bottom: 15px;
        visibility: hidden;
...
# --- CONFIGURACIÓN DE COLORES RECUPERADA (Lila y Pasteles) ---
CONFIG_PROB = {
...
def render_card(seccion, item, idx):
    ...
    # Línea superior con el color de la sección
    st.markdown(f'<div style="background-color: {CONFIG_PROB[seccion]["color"]}; height: 10px; border-radius: 10px 10px 0 0;"></div>', unsafe_allow_html=True)
...
if not any(st.session_state['arbol_tarjetas'].values()):
    ...
    # 1. SECCIÓN EFECTOS (Lila y Azul con Suelo Fijo)
    ...
    if ef_dir:
        # Cálculo para nivelar los padres en la base
        conteos = []
        for ed in ef_dir:
    ...
    st.markdown("---")
    # 3. SECCIÓN CAUSAS (Amarillo y Naranja)
    ...
    if ca_dir:
        ...
        for i, cd in enumerate(ca_dir):
            with cols_ca[i]:
                # Aquí el padre queda arriba y los hijos crecen hacia abajo
