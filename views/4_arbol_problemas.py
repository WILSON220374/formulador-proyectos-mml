...
# Borra de la memoria cualquier ficha que no tenga texto real antes de empezar
if 'arbol_tarjetas' in st.session_state:
    for seccion in st.session_state['arbol_tarjetas']:
        ...
        if isinstance(lista, list):
            # Solo mantenemos fichas que tengan texto, más de 2 caracteres y que no sean "None"
...
st.markdown("""
    ...
    /* Estilo de las tarjetas en el Constructor */
...
with col_t:
    ...
    st.progress(1.0 if hay_datos else 0.0, text="Árbol Activo" if hay_datos else "Esperando Problema Central")
...
# --- MOTOR DE DIBUJO (CON FILTRO ESTRICTO) ---
def generar_grafo_problemas():
    datos = st.session_state.get('arbol_tarjetas', {})
    # Solo procesamos el Problema Principal si es válido
    pc = [it for it in datos.get("Problema Principal", []) if it.get('texto') and str(it.get('texto')).upper() != "NONE"]
    if not pc: return None

    dot = graphviz.Digraph(format='png')
    # Forzamos un tamaño de lienzo para que el árbol no se pierda en espacios vacíos
    ...
    for tipo, id_p, p_key, edge_dir, col in ramas:
        # Filtro: Ignorar cualquier ficha sin texto o que diga "None"
...
def render_card(seccion, item, idx):
    ...
    if st.button("🗑️", key=f"btn_{id_u}", help="Eliminar ficha"):
    ...
    with st.form("crear_ficha", clear_on_submit=True):
        ...
        if st.form_submit_button("Generar Ficha") and texto_input.strip():
            if len(texto_input.strip()) > 2:
                ...
                st.error("Texto demasiado corto.")
...
with tab1:
    if not hay_datos:
        st.info("🎯 Comience definiendo el Problema Principal en el menú lateral.")
    else:
        # Aquí solo se muestra el gráfico limpio
...
with tab2:
    if not hay_datos:
        st.info("💡 Use el menú lateral para construir la estructura de su árbol.")
    else:
        st.subheader("📋 Panel de Edición")
        
        def mostrar_seccion_dinamica(tipo_padre, tipo_hijo):
            # Filtramos para que no aparezcan cuadros vacíos en la edición
            ...
            for i, p in enumerate(padres):
                with cols[i]:
                    ...
                    # Renderizar hijos justo debajo de su padre
