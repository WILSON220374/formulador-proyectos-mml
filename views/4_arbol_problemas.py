...
# --- SISTEMA DE LIMPIEZA AUTOMÁTICA (FILTRO SANITARIO) ---
...
with col_t:
    ...
    st.markdown('Construcción lógica y visualización profesional del diagnóstico.', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("⚠️ Zona de Peligro"):
        st.write("Eliminar todos los datos del árbol.")
...
with tab1:
    if not pc_data: st.info("🎯 Defina el Problema Principal.")
    else:
        g = generar_grafo_problemas()
        if g: st.image(g.pipe(format='png'), use_container_width=True)
with tab2:
    if not pc_data: st.info("💡 Use el menú lateral.")
    else:
