...
with st.sidebar:
    ...
    def generar_png_arbol():
        ...
        for seccion, conf in CONFIG.items():
            # FILTRADO CRÍTICO: El PNG ahora sigue la misma lógica que la pantalla
            ...
            for i, item in enumerate(items):
                ...
                rect = plt.Rectangle((x-1.1, y_base-0.4), 2.2, max(0.6, num_lineas * 0.18), 
                                     facecolor=conf["color"], edgecolor='#333', lw=1.2, zorder=2)
                ax.add_patch(rect)
...
def render_simple(nombre):
    ...
    with col_c:
        ...
        if items:
            ...
            # Papelera restaurada para Problema Principal
...
def render_rama(nombre_padre, nombre_hijo, inversion=False):
        ...
        with col_c:
            if not padres and not es_hijo: st.caption(f"Cree un {nombre_padre} primero.")
                ...
                for i, p_txt in enumerate(padres):
                    with cols[i]:
                        if es_hijo:
                            # Mostramos solo hijos válidos vinculados
    ...
    # SECCIÓN ESPECIAL: Limpieza de huérfanos (Fantasmas como tu ficha 'a')
    ...
    if huerfanos:
        with st.expander("⚠️ Fichas sin vínculo (Fantasmas)"):
            for h in huerfanos:
                ...
                st.write(f"- {txt}")
                if st.button(f"Eliminar ficha fantasma: {txt[:20]}...", key=f"clean_{txt}"):
