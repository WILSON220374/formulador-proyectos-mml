...
def generar_word():
  ...
  # 1. Primero intenta poner la imagen (el dibujo)
    if not imagen_obj_insertada and arbol_obj_datos:
        ...
        if img_obj_recreada:
            try:
                p_arbol_obj = doc.add_paragraph()
                p_arbol_obj.alignment = WD_ALIGN_PARAGRAPH.CENTER
                # USAMOS LA IMAGEN DIRECTA (SIN EL io.BytesIO EXTRA)
                p_arbol_obj.add_run().add_picture(img_obj_recreada, width=Inches(6.0))
                imagen_obj_insertada = True
            except Exception:
                doc.add_paragraph("[Aviso: No se pudo renderizar la imagen del árbol]")

    # 2. JUSTO DEBAJO, mantienes el texto oficial (¡NO LO QUITES!)
